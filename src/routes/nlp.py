...
async def index_project_auto(request: Request, push_request: PushRequest):
    ...
    async with request.app.db_client() as session:
        result = await session.execute(
            select(Project).where(Project.user_id == request.state.user.user_id)
        )
...
async def answer_with_api_key(request: Request, search_request: SearchRequest):
    ...
    # project is set by middleware from X-API-KEY header
